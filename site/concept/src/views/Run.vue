    ...
    <div class="field is-grouped">
      <div class="field has-addons is-expanded" style="margin-bottom: 0;">
        ...
        <div class="control is-expanded">
          <a class="button is-static" style="width: 100%; justify-content: left;">
            {{ descs[active] }}
          </a>
        </div>
      </div>
...
for (var k in c.data) {
    if (c.data.hasOwnProperty(k)) {
        if (!graphs[k]) {
            console.log('Key <'+k+'> not found in graphs list; skipping description.')
        } else {
            descs[k] = c.data[k];
        }
    }
